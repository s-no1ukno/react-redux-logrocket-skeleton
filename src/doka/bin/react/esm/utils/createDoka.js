...
const filteredKeys = ['id', 'className']
...
const allowedProps = Object.keys(OptionTypes).filter(
  key => !filteredKeys.includes(key)
)

export const propsToElementAttributes = (
  base = {},
  { id, style, className }
) => {
  return {
...
const getOptionsFromProps = (props, filter) =>
  Object.keys(props)
    .filter(prop =>
      filter
        ? allowedProps.find(key => key.toLowerCase() === prop.toLowerCase())
        : true
    )
    .reduce((options, key) => {
...
export const updateDoka = (instance, props) =>
  instance.setOptions(getOptionsFromProps(props, true))
